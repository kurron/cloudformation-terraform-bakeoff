...
* *environment* - logical grouping the resource belongs to, e.g. `production`
...
IMPORTANT: when creation the Bastion boxes, you must've specified your public ip address as the allowed network or the firewall won't allow access.  If you didn't do this, you can go into the AWS console and adjust the Bastion box's security group, adding your ip address.
...
=== Terraform Failures
I don't think I have the resource dependencies completely defined so if you get an error about something not existing, just run the script again and it should cure itself.
