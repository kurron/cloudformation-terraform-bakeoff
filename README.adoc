...
NOTE: after I completed my evaluation, I discovered https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/intrinsic-function-reference-importvalue.html[Fn::ImportValue] which allows one stack to reference the output value of another stack.  Output variables must be unique within the account.  Having the operator paste in values by hand is likely not necessary.
